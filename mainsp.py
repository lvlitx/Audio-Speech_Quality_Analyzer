...
SPEECH_TIMEOUT = 2.0  # seconds of silence before stopping recording
MIN_SPEECH_DURATION = 0.5  # minimum seconds of speech to process
MAX_SPEECH_DURATION = 30.0  # maximum seconds to record continuously

# Global variables
audio_queue = queue.Queue()
is_listening = True
speech_buffer = deque(maxlen=int(MAX_SPEECH_DURATION / CHUNK_DURATION))

def audio_callback(indata, frames, time, status):
    """Callback function for continuous audio input"""
    if status:
        print(f"Audio callback status: {status}")
    # Convert to the format expected by the processing thread
    audio_queue.put(indata.copy())

def detect_speech_in_chunk(audio_chunk, vad):
    """Check if there's speech in an audio chunk using WebRTC VAD"""
    # Convert float32 to int16 PCM
    pcm_data = (audio_chunk.flatten() * 32767).astype("int16").tobytes()
    ...
    return speech_frames > (total_frames * 0.5) if total_frames > 0 else False
...
def process_audio():
    ...
    while is_listening:
        try:
            ...
            has_speech = detect_speech_in_chunk(audio_chunk, vad)
            ...
            if has_speech:
                if not is_speaking:
                    ...
                    speech_audio = list(speech_buffer)[-10:]  # Last ~5 seconds
                else:
                ...
                if is_speaking:
                    ...
                    # Continue adding audio during silence period
                    speech_audio.append(audio_chunk)
                    ...
                    if time.time() - silence_start >= SPEECH_TIMEOUT:
                        ...
                        if speech_duration >= MIN_SPEECH_DURATION:
                            print(f"⏹️  Speech ended ({speech_duration:.1f}s), transcribing...")
                            
                            # Combine all audio chunks
                            full_audio = np.concatenate(speech_audio, axis=0).flatten()
                            
                            # Transcribe
                            transcript = transcribe_audio(full_audio, model)
                            ...
                            print(f"⚠️  Speech too short ({speech_duration:.1f}s), ignoring")
            ...
            if is_speaking and speech_start_time:
                if time.time() - speech_start_time >= MAX_SPEECH_DURATION:
                    print(f"⚠️  Maximum recording duration ({MAX_SPEECH_DURATION}s) reached, processing...")
...
def main():
    """Main function to start continuous listening"""
    global is_listening
